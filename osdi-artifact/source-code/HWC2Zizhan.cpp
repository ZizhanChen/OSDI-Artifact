...
#undef LOG_TAG
#define LOG_TAG "HWC2Zizhan"
#define ATRACE_TAG ATRACE_TAG_GRAPHICS

#include "HWC2Zizhan.h"
#include "ComposerHalZizhan.h"
...
namespace HWC2Zizhan {

namespace Hwc2Zizhan = android::Hwc2Zizhan;
...
inline bool hasMetadataKey(const std::set<Hwc2Zizhan::PerFrameMetadataKey>& keys,
                           const Hwc2Zizhan::PerFrameMetadataKey& key) {
    return keys.find(key) != keys.end();
}

class ComposerCallbackBridge : public Hwc2Zizhan::IComposerCallback {
public:
    ComposerCallbackBridge(ComposerCallback* callback, int32_t sequenceId)
            : mCallback(callback), mSequenceId(sequenceId) {}

    Return<void> onHotplug(Hwc2Zizhan::Display display,
                           IComposerCallback::Connection conn) override
    {
        HWC2Zizhan::Connection connection = static_cast<HWC2Zizhan::Connection>(conn);
        mCallback->onHotplugReceived(mSequenceId, display, connection);
        return Void();
    }

    Return<void> onRefresh(Hwc2Zizhan::Display display) override
    {
        mCallback->onRefreshReceived(mSequenceId, display);
        return Void();
    }

    Return<void> onVsync(Hwc2Zizhan::Display display, int64_t timestamp) override
    {
...
Device::Device(std::unique_ptr<android::Hwc2Zizhan::Composer> composer) : mComposer(std::move(composer)) {
    loadCapabilities();
}
...
void Device::onHotplug(hwc2_display_t displayId, Connection connection) {
    if (connection == Connection::Connected) {
        ...
        auto intError = mComposer->getDisplayType(displayId,
                reinterpret_cast<Hwc2Zizhan::IComposerClient::DisplayType *>(
                        &displayType));
        auto error = static_cast<Error>(intError);
        if (error != Error::None) {
...
namespace implZizhan {
Display::Display(android::Hwc2Zizhan::Composer& composer,
...
Display::~Display() {
    ...
    if (mType == DisplayType::Virtual) {
        ...
        auto error = setVsyncEnabled(HWC2Zizhan::Vsync::Disable);
        if (error != Error::None) {
...
// Required by HWC2Zizhan display
...
Error Display::createLayer(HWC2Zizhan::Layer** outLayer) {
    if (!outLayer) {
...
Error Display::destroyLayer(HWC2Zizhan::Layer* layer) {
    if (!layer) {
        return Error::BadParameter;
    }
...
Error Display::getChangedCompositionTypes(std::unordered_map<HWC2Zizhan::Layer*, Composition>* outTypes) {
    std::vector<Hwc2Zizhan::Layer> layerIds;
    std::vector<Hwc2Zizhan::IComposerClient::Composition> types;
...
int32_t Display::getSupportedPerFrameMetadata() const
{
    ...
    std::vector<Hwc2Zizhan::PerFrameMetadataKey> tmpKeys = mComposer.getPerFrameMetadataKeys(mId);
    std::set<Hwc2Zizhan::PerFrameMetadataKey> keys(tmpKeys.begin(), tmpKeys.end());
    ...
    // SMPTE2086
    if (hasMetadataKey(keys, Hwc2Zizhan::PerFrameMetadataKey::DISPLAY_RED_PRIMARY_X) &&
        hasMetadataKey(keys, Hwc2Zizhan::PerFrameMetadataKey::DISPLAY_RED_PRIMARY_Y) &&
        hasMetadataKey(keys, Hwc2Zizhan::PerFrameMetadataKey::DISPLAY_GREEN_PRIMARY_X) &&
        hasMetadataKey(keys, Hwc2Zizhan::PerFrameMetadataKey::DISPLAY_GREEN_PRIMARY_Y) &&
        hasMetadataKey(keys, Hwc2Zizhan::PerFrameMetadataKey::DISPLAY_BLUE_PRIMARY_X) &&
        hasMetadataKey(keys, Hwc2Zizhan::PerFrameMetadataKey::DISPLAY_BLUE_PRIMARY_Y) &&
        hasMetadataKey(keys, Hwc2Zizhan::PerFrameMetadataKey::WHITE_POINT_X) &&
        hasMetadataKey(keys, Hwc2Zizhan::PerFrameMetadataKey::WHITE_POINT_Y) &&
        hasMetadataKey(keys, Hwc2Zizhan::PerFrameMetadataKey::MAX_LUMINANCE) &&
        hasMetadataKey(keys, Hwc2Zizhan::PerFrameMetadataKey::MIN_LUMINANCE)) {
        supportedPerFrameMetadata |= HdrMetadata::Type::SMPTE2086;
    }
    // CTA861_3
    if (hasMetadataKey(keys, Hwc2Zizhan::PerFrameMetadataKey::MAX_CONTENT_LIGHT_LEVEL) &&
        hasMetadataKey(keys, Hwc2Zizhan::PerFrameMetadataKey::MAX_FRAME_AVERAGE_LIGHT_LEVEL)) {
        supportedPerFrameMetadata |= HdrMetadata::Type::CTA861_3;
    }

    // HDR10PLUS
    if (hasMetadataKey(keys, Hwc2Zizhan::PerFrameMetadataKey::HDR10_PLUS_SEI)) {
...
Error Display::getRequests(HWC2Zizhan::DisplayRequest* outDisplayRequests,
                           std::unordered_map<HWC2Zizhan::Layer*, LayerRequest>* outLayerRequests) {
...
Error Display::getHdrCapabilities(HdrCapabilities* outCapabilities) const
{
    ...
    std::vector<Hwc2Zizhan::Hdr> types;
    auto intError = mComposer.getHdrCapabilities(mId, &types,
            &maxLuminance, &maxAverageLuminance, &minLuminance);
    auto error = static_cast<HWC2Zizhan::Error>(intError);
...
Error Display::getReleaseFences(std::unordered_map<HWC2Zizhan::Layer*, sp<Fence>>* outFences) const {
    std::vector<Hwc2Zizhan::Layer> layerIds;
    ...
    std::unordered_map<HWC2Zizhan::Layer*, sp<Fence>> releaseFences;
        ...
        const sp<Fence>& acquireFence, Dataspace dataspace)
{
    ...
    auto intError = mComposer.setClientTarget(mId, slot, target,
            fenceFd, dataspace, std::vector<Hwc2Zizhan::IComposerClient::Rect>());
    return static_cast<Error>(intError);
}
        ...
        android_color_transform_t hint)
{
    auto intError = mComposer.setColorTransform(mId,
            matrix.asArray(), static_cast<Hwc2Zizhan::ColorTransform>(hint));
    return static_cast<Error>(intError);
}
...
{
    auto intMode = static_cast<Hwc2Zizhan::IComposerClient::PowerMode>(mode);
    auto intError = mComposer.setPowerMode(mId, intMode);

    if (mode == PowerMode::On) {
        std::call_once(mDisplayCapabilityQueryFlag, [this]() {
            std::vector<Hwc2Zizhan::DisplayCapability> tmpCapabilities;
            auto error =
...
{
    auto intEnabled = static_cast<Hwc2Zizhan::IComposerClient::Vsync>(enabled);
...
int32_t Display::getAttribute(hwc2_config_t configId, Attribute attribute)
{
    ...
    auto intError = mComposer.getDisplayAttribute(mId, configId,
            static_cast<Hwc2Zizhan::IComposerClient::Attribute>(attribute),
            &value);
    auto error = static_cast<Error>(intError);
    if (error != Error::None) {
...
void Display::loadConfigs()
{
    ...
    std::vector<Hwc2Zizhan::Config> configIds;
...
HWC2Zizhan::Layer* Display::getLayerById(hwc2_layer_t id) const {
...
Layer::Layer(android::Hwc2Zizhan::Composer& composer, const std::unordered_set<Capability>& capabilities,
...
Error Layer::setSurfaceDamage(const Region& damage)
{
    ...
    // rects for HWC
    Hwc2Zizhan::Error intError = Hwc2Zizhan::Error::NONE;
    if (damage.isRect() && damage.getBounds() == Rect::INVALID_RECT) {
        intError = mComposer.setLayerSurfaceDamage(mDisplayId,
                mId, std::vector<Hwc2Zizhan::IComposerClient::Rect>());
    } else {
        size_t rectCount = 0;
        auto rectArray = damage.getArray(&rectCount);

        std::vector<Hwc2Zizhan::IComposerClient::Rect> hwcRects;
...
{
    auto intMode = static_cast<Hwc2Zizhan::IComposerClient::BlendMode>(mode);
...
{
    Hwc2Zizhan::IComposerClient::Color hwcColor{color.r, color.g, color.b, color.a};
...
{
    auto intType = static_cast<Hwc2Zizhan::IComposerClient::Composition>(type);
        ...
        const android::HdrMetadata& metadata)
{
    ...
    std::vector<Hwc2Zizhan::PerFrameMetadata> perFrameMetadatas;
    if (validTypes & HdrMetadata::SMPTE2086) {
        perFrameMetadatas.insert(perFrameMetadatas.end(),
                                 {{Hwc2Zizhan::PerFrameMetadataKey::DISPLAY_RED_PRIMARY_X,
                                   mHdrMetadata.smpte2086.displayPrimaryRed.x},
                                  {Hwc2Zizhan::PerFrameMetadataKey::DISPLAY_RED_PRIMARY_Y,
                                   mHdrMetadata.smpte2086.displayPrimaryRed.y},
                                  {Hwc2Zizhan::PerFrameMetadataKey::DISPLAY_GREEN_PRIMARY_X,
                                   mHdrMetadata.smpte2086.displayPrimaryGreen.x},
                                  {Hwc2Zizhan::PerFrameMetadataKey::DISPLAY_GREEN_PRIMARY_Y,
                                   mHdrMetadata.smpte2086.displayPrimaryGreen.y},
                                  {Hwc2Zizhan::PerFrameMetadataKey::DISPLAY_BLUE_PRIMARY_X,
                                   mHdrMetadata.smpte2086.displayPrimaryBlue.x},
                                  {Hwc2Zizhan::PerFrameMetadataKey::DISPLAY_BLUE_PRIMARY_Y,
                                   mHdrMetadata.smpte2086.displayPrimaryBlue.y},
                                  {Hwc2Zizhan::PerFrameMetadataKey::WHITE_POINT_X,
                                   mHdrMetadata.smpte2086.whitePoint.x},
                                  {Hwc2Zizhan::PerFrameMetadataKey::WHITE_POINT_Y,
                                   mHdrMetadata.smpte2086.whitePoint.y},
                                  {Hwc2Zizhan::PerFrameMetadataKey::MAX_LUMINANCE,
                                   mHdrMetadata.smpte2086.maxLuminance},
                                  {Hwc2Zizhan::PerFrameMetadataKey::MIN_LUMINANCE,
                                   mHdrMetadata.smpte2086.minLuminance}});
    }

    if (validTypes & HdrMetadata::CTA861_3) {
        perFrameMetadatas.insert(perFrameMetadatas.end(),
                                 {{Hwc2Zizhan::PerFrameMetadataKey::MAX_CONTENT_LIGHT_LEVEL,
                                   mHdrMetadata.cta8613.maxContentLightLevel},
                                  {Hwc2Zizhan::PerFrameMetadataKey::MAX_FRAME_AVERAGE_LIGHT_LEVEL,
    ...
    if (validTypes & HdrMetadata::HDR10PLUS) {
        std::vector<Hwc2Zizhan::PerFrameMetadataBlob> perFrameMetadataBlobs;
        perFrameMetadataBlobs.push_back(
                {Hwc2Zizhan::PerFrameMetadataKey::HDR10_PLUS_SEI, mHdrMetadata.hdr10plus});
...
{
    Hwc2Zizhan::IComposerClient::Rect hwcRect{frame.left, frame.top,
...
{
    Hwc2Zizhan::IComposerClient::FRect hwcRect{
...
{
    auto intTransform = static_cast<Hwc2Zizhan::Transform>(transform);
