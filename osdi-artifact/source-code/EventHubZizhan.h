...
//
#ifndef _RUNTIME_EVENT_HUB_ZIZHAN_H
#define _RUNTIME_EVENT_HUB_ZIZHAN_H
...
#include "TouchVideoDeviceZizhan.h"
...
struct RawAbsoluteAxisInfoZizhan {
...
enum {
    ...
    INPUT_DEVICE_CLASS_KEYBOARD_ZIZHAN      = 0x00000001,

    /* The input device is an alpha-numeric keyboard (not just a dial pad). */
    INPUT_DEVICE_CLASS_ALPHAKEY_ZIZHAN      = 0x00000002,

    /* The input device is a touchscreen or a touchpad (either single-touch or multi-touch). */
    INPUT_DEVICE_CLASS_TOUCH_ZIZHAN         = 0x00000004,

    /* The input device is a cursor device such as a trackball or mouse. */
    INPUT_DEVICE_CLASS_CURSOR_ZIZHAN        = 0x00000008,

    /* The input device is a multi-touch touchscreen. */
    INPUT_DEVICE_CLASS_TOUCH_MT_ZIZHAN      = 0x00000010,

    /* The input device is a directional pad (implies keyboard, has DPAD keys). */
    INPUT_DEVICE_CLASS_DPAD_ZIZHAN          = 0x00000020,

    /* The input device is a gamepad (implies keyboard, has BUTTON keys). */
    INPUT_DEVICE_CLASS_GAMEPAD_ZIZHAN       = 0x00000040,

    /* The input device has switches. */
    INPUT_DEVICE_CLASS_SWITCH_ZIZHAN        = 0x00000080,

    /* The input device is a joystick (implies gamepad, has joystick absolute axes). */
    INPUT_DEVICE_CLASS_JOYSTICK_ZIZHAN      = 0x00000100,

    /* The input device has a vibrator (supports FF_RUMBLE). */
    INPUT_DEVICE_CLASS_VIBRATOR_ZIZHAN      = 0x00000200,

    /* The input device has a microphone. */
    INPUT_DEVICE_CLASS_MIC_ZIZHAN           = 0x00000400,

    /* The input device is an external stylus (has data we want to fuse with touch data). */
    INPUT_DEVICE_CLASS_EXTERNAL_STYLUS_ZIZHAN = 0x00000800,

    /* The input device has a rotary encoder */
    INPUT_DEVICE_CLASS_ROTARY_ENCODER_ZIZHAN = 0x00001000,

    /* The input device has a mouse. */
    INPUT_DEVICE_CLASS_KEYMOUSE_ZIZHAN       = 0x00004000,

    /* The input device is virtual (not a real device, not part of UI configuration). */
    INPUT_DEVICE_CLASS_VIRTUAL_ZIZHAN       = 0x40000000,

    /* The input device is external (not built-in). */
    INPUT_DEVICE_CLASS_EXTERNAL_ZIZHAN      = 0x80000000,
};
 ...
 */
class EventHubInterfaceZizhan : public virtual RefBase {
protected:
    EventHubInterfaceZizhan() { }
    virtual ~EventHubInterfaceZizhan() { }
...
public:
    ...
    virtual status_t getAbsoluteAxisInfo(int32_t deviceId, int axis,
            RawAbsoluteAxisInfoZizhan* outAxisInfo) const = 0;
     ...
     */
    virtual size_t getEvents(int timeoutMillis, RawEventZizhan* buffer, size_t bufferSize) = 0;
...
class EventHubZizhan : public EventHubInterfaceZizhan
{
public:
    EventHubZizhan();
    ...
    virtual size_t getEvents(int timeoutMillis, RawEventZizhan* buffer, size_t bufferSize);
...
private:
    struct Device {
        ...
        std::unique_ptr<TouchVideoDeviceZizhan> videoDevice;
    ...
    void registerVideoDeviceForEpollLocked(const TouchVideoDeviceZizhan& videoDevice);
    status_t unregisterDeviceFromEpollLocked(Device* device);
    void unregisterVideoDeviceFromEpollLocked(const TouchVideoDeviceZizhan& videoDevice);
     ...
     * actively streaming.
     */
    std::vector<std::unique_ptr<TouchVideoDeviceZizhan>> mUnattachedVideoDevices;
...
#endif // _RUNTIME_EVENT_HUB_ZIZHAN_H
