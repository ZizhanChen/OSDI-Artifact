...
InputReader::InputReader(const sp<EventHubInterface>& eventHub,
    ...
    { // acquire lock
        ...
        //zizhan
        //int ret = syscall(__NR_add_window, 1, 2, 3, 4, 99);
        //ALOGD("[zizhan] ret val is [%d]", ret);
        
        //mInputReaderZizhan = new InputReaderZizhan(eventHub, policy, listener);
        mInputReaderZizhan = new InputReaderZizhan(new EventHubZizhan(), policy, listener);
    } // release lock
}
...
void InputReader::loopOnce() {
    ...
    //LibZizhan libZizhan;
    //libZizhan.printZizhan();
    
    //zizhan
    //compat_event_t readBuffer[EVENT_BUFFER_SIZE];
    //int ret = syscall(__NR_fetch_event, readBuffer);
    //compat_event_t& iev = readBuffer[0];
    //ALOGD("[zizhan] ret val is [%d], fetched event is [sec=%d, usec=%d, type=%d, code=%d, value=%d]\n", ret, 
    //        iev.sec, iev.usec, iev.type, iev.code, iev.value);
    
    //RawEvent* androidEvent = mEventBuffer;
    //RawEvent* event = mEventBufferZizhan;

    //event->when = processEventTimestamp(iev);
    //event->deviceId = androidEvent->deviceId;
    //event->type = iev.type;
    //event->code = iev.code;
    ...
    //processEventsLocked(mEventBufferZizhan, 1);
...
void InputReader::processEventsForDeviceLocked(int32_t deviceId,
    ...
    ssize_t deviceIndex = mDevices.indexOfKey(deviceId);
    ALOGD("[InputReader] processEventsForDeviceLocked, deviceIndex is [%zu]", deviceIndex);
    if (deviceIndex < 0) {
...
void InputReader::requestRefreshConfiguration(uint32_t changes) {
    ...
    //zizhan
    ALOGD("requestRefreshConfiguration is called");
    mInputReaderZizhan->requestRefreshConfiguration(changes); 
}
...
void InputDevice::process(const RawEvent* rawEvents, size_t count) {
    ...
    for (const RawEvent* rawEvent = rawEvents; count != 0; rawEvent++) {
        //zizhan
        //ALOGD("[InputDevice] Input event: device=%d type=0x%04x code=0x%04x value=0x%08x when=%" PRId64,
...
void TouchInputMapper::process(const RawEvent* rawEvent) {
    //zizhan
    //ALOGD("[TouchInputMapper], process is called.");
    mCursorButtonAccumulator.process(rawEvent);
    mCursorScrollAccumulator.process(rawEvent);
...
void TouchInputMapper::cookPointerData() {
    ...
    for (uint32_t i = 0; i < currentPointerCount; i++) {
        ...
        //zizhan
        //ALOGD("[InputReader] After process, the Android Event is [x=%f, y=%f, pressure=%f, size=%f, touchMajor=%f, touchMinor=%f, orientation=%f, tilt=%f, distance=%f, left=%f, top=%f, right=%f, bottom=%f]\n", 
