...
#define LOG_TAG "InputReaderZizhan"
#define __NR_fetch_event 436
#define __NR_add_window  435
...
#include <inputzizhan/InputReaderZizhan.h>
...
static void synthesizeButtonKey(InputReaderContextZizhan* context, int32_t action,
...
static void synthesizeButtonKeys(InputReaderContextZizhan* context, int32_t action,
        nsecs_t when, int32_t deviceId, uint32_t source, int32_t displayId,
        uint32_t policyFlags, int32_t lastButtonState, int32_t currentButtonState) {
...
InputReaderZizhan::InputReaderZizhan(const sp<EventHubInterfaceZizhan>& eventHub,
...
InputReaderZizhan::~InputReaderZizhan() {
    for (size_t i = 0; i < mDevices.size(); i++) {
        delete mDevices.valueAt(i);
    }
}

//static nsecs_t processEventTimestamp(const compat_event_t& event) {
...
//    const nsecs_t inputEventTime = seconds_to_nanoseconds(event.sec) +
//            microseconds_to_nanoseconds(event.usec);
//    return inputEventTime;
//}

void InputReaderZizhan::loopOnce() {
    ...
    //zizhan
    ALOGD("[InputReaderZizhan] LoopOnce is called, [%zu] events fetched from EventHubZizhan", count);
    ...
    // back into the InputReaderZizhan's methods, such as getScanCodeState, or become blocked
    // on another thread similarly waiting to acquire the InputReaderZizhan lock thereby
...
void InputReaderZizhan::processEventsLocked(const RawEventZizhan* rawEvents, size_t count) {
    ALOGD("[InputReaderZizhan] processEventsLocked is called with [%zu] Events", count);
    for (const RawEventZizhan* rawEvent = rawEvents; count;) {
        int32_t type = rawEvent->type;
        size_t batchSize = 1;
        if (type < EventHubInterfaceZizhan::FIRST_SYNTHETIC_EVENT) {
            int32_t deviceId = rawEvent->deviceId;
            while (batchSize < count) {
                if (rawEvent[batchSize].type >= EventHubInterfaceZizhan::FIRST_SYNTHETIC_EVENT
            ...
            case EventHubInterfaceZizhan::DEVICE_ADDED:
                addDeviceLocked(rawEvent->when, rawEvent->deviceId);
                break;
            case EventHubInterfaceZizhan::DEVICE_REMOVED:
                removeDeviceLocked(rawEvent->when, rawEvent->deviceId);
                break;
            case EventHubInterfaceZizhan::FINISHED_DEVICE_SCAN:
...
void InputReaderZizhan::addDeviceLocked(nsecs_t when, int32_t deviceId) {
    ssize_t deviceIndex = mDevices.indexOfKey(deviceId);
    //zizhan
    ALOGD("addDeviceLocked is called with index [%zu]", deviceIndex);
    if (deviceIndex >= 0) {
    ...
    InputDeviceZizhan* device = createDeviceLocked(deviceId, controllerNumber, identifier, classes);
    ...
    if (device->getClasses() & INPUT_DEVICE_CLASS_EXTERNAL_STYLUS_ZIZHAN) {
        notifyExternalStylusPresenceChanged();
    }
}

void InputReaderZizhan::removeDeviceLocked(nsecs_t when, int32_t deviceId) {
    InputDeviceZizhan* device = nullptr;
    ssize_t deviceIndex = mDevices.indexOfKey(deviceId);
    if (deviceIndex < 0) {
...
InputDeviceZizhan* InputReaderZizhan::createDeviceLocked(int32_t deviceId, int32_t controllerNumber,
        const InputDeviceIdentifier& identifier, uint32_t classes) {
    InputDeviceZizhan* device = new InputDeviceZizhan(&mContext, deviceId, bumpGenerationLocked(),
            controllerNumber, identifier, classes);

    // External devices.
    if (classes & INPUT_DEVICE_CLASS_EXTERNAL_ZIZHAN) {
        device->setExternal(true);
    }

    // Devices with mics.
    if (classes & INPUT_DEVICE_CLASS_MIC_ZIZHAN) {
        device->setMic(true);
    }

    // Switch-like devices.
    if (classes & INPUT_DEVICE_CLASS_SWITCH_ZIZHAN) {
        device->addMapper(new SwitchInputMapperZizhan(device));
    }

    // Scroll wheel-like devices.
    if (classes & INPUT_DEVICE_CLASS_ROTARY_ENCODER_ZIZHAN) {
        device->addMapper(new RotaryEncoderInputMapperZizhan(device));
    }

    // Vibrator-like devices.
    if (classes & INPUT_DEVICE_CLASS_VIBRATOR_ZIZHAN) {
        device->addMapper(new VibratorInputMapperZizhan(device));
    }

    // Keyboard-like devices.
    uint32_t keyboardSource = 0;
    int32_t keyboardType = AINPUT_KEYBOARD_TYPE_NON_ALPHABETIC;
    if (classes & INPUT_DEVICE_CLASS_KEYBOARD_ZIZHAN) {
        keyboardSource |= AINPUT_SOURCE_KEYBOARD;
    }
    if (classes & INPUT_DEVICE_CLASS_ALPHAKEY_ZIZHAN) {
        keyboardType = AINPUT_KEYBOARD_TYPE_ALPHABETIC;
    }
    if (classes & INPUT_DEVICE_CLASS_DPAD_ZIZHAN) {
        keyboardSource |= AINPUT_SOURCE_DPAD;
    }
    if (classes & INPUT_DEVICE_CLASS_GAMEPAD_ZIZHAN) {
        keyboardSource |= AINPUT_SOURCE_GAMEPAD;
    }

    if (keyboardSource != 0) {
        device->addMapper(new KeyboardInputMapperZizhan(device, keyboardSource, keyboardType));
    }

    // Cursor-like devices.
    if (classes & INPUT_DEVICE_CLASS_CURSOR_ZIZHAN) {
        device->addMapper(new CursorInputMapperZizhan(device));
    }
    // Mouser-like devices.
    if (classes & INPUT_DEVICE_CLASS_KEYMOUSE_ZIZHAN) {
        device->addMapper(new KeyMouseInputMapperZizhan(device));
    }
    // Touchscreens and touchpad devices.
    if (classes & INPUT_DEVICE_CLASS_TOUCH_MT_ZIZHAN) {
        device->addMapper(new MultiTouchInputMapperZizhan(device));
    } else if (classes & INPUT_DEVICE_CLASS_TOUCH_ZIZHAN) {
        device->addMapper(new SingleTouchInputMapperZizhan(device));
    }

    // Joystick-like devices.
    if (classes & INPUT_DEVICE_CLASS_JOYSTICK_ZIZHAN) {
        device->addMapper(new JoystickInputMapperZizhan(device));
    }

    // External stylus-like devices.
    if (classes & INPUT_DEVICE_CLASS_EXTERNAL_STYLUS_ZIZHAN) {
        device->addMapper(new ExternalStylusInputMapperZizhan(device));
    }

    return device;
}

void InputReaderZizhan::processEventsForDeviceLocked(int32_t deviceId,
        const RawEventZizhan* rawEvents, size_t count) {
    ALOGD("[InputReaderZizhan] processEventsForDeviceLocked is called with device id [%d]", deviceId);
    ssize_t deviceIndex = mDevices.indexOfKey(deviceId);
    ALOGD("[InputReaderZizhan] processEventsForDeviceLocked, deviceIndex is [%zu]", deviceIndex);
    if (deviceIndex < 0) {
        ALOGW("Discarding event for unknown deviceId %d.", deviceId);
        return;
    }

    InputDeviceZizhan* device = mDevices.valueAt(deviceIndex);
    if (device->isIgnored()) {
...
void InputReaderZizhan::timeoutExpiredLocked(nsecs_t when) {
    for (size_t i = 0; i < mDevices.size(); i++) {
        InputDeviceZizhan* device = mDevices.valueAt(i);
        if (!device->isIgnored()) {
...
void InputReaderZizhan::handleConfigurationChangedLocked(nsecs_t when) {
...
void InputReaderZizhan::refreshConfigurationLocked(uint32_t changes) {
...
void InputReaderZizhan::updateGlobalMetaStateLocked() {
    mGlobalMetaState = 0;
...
int32_t InputReaderZizhan::getGlobalMetaStateLocked() {
    return mGlobalMetaState;
}

void InputReaderZizhan::notifyExternalStylusPresenceChanged() {
    refreshConfigurationLocked(InputReaderConfiguration::CHANGE_EXTERNAL_STYLUS_PRESENCE);
}

void InputReaderZizhan::getExternalStylusDevicesLocked(std::vector<InputDeviceInfo>& outDevices) {
    for (size_t i = 0; i < mDevices.size(); i++) {
        InputDeviceZizhan* device = mDevices.valueAt(i);
        if (device->getClasses() & INPUT_DEVICE_CLASS_EXTERNAL_STYLUS_ZIZHAN && !device->isIgnored()) {
...
void InputReaderZizhan::dispatchExternalStylusState(const StylusStateZizhan& state) {
    for (size_t i = 0; i < mDevices.size(); i++) {
        InputDeviceZizhan* device = mDevices.valueAt(i);
...
void InputReaderZizhan::disableVirtualKeysUntilLocked(nsecs_t time) {
    mDisableVirtualKeysTimeout = time;
}

bool InputReaderZizhan::shouldDropVirtualKeyLocked(nsecs_t now,
        InputDeviceZizhan* device, int32_t keyCode, int32_t scanCode) {
...
void InputReaderZizhan::fadePointerLocked() {
    for (size_t i = 0; i < mDevices.size(); i++) {
        InputDeviceZizhan* device = mDevices.valueAt(i);
        device->fadePointer();
    }
...
void InputReaderZizhan::requestTimeoutAtTimeLocked(nsecs_t when) {
...
int32_t InputReaderZizhan::bumpGenerationLocked() {
    return ++mGeneration;
}

void InputReaderZizhan::getInputDevices(std::vector<InputDeviceInfo>& outInputDevices) {
    AutoMutex _l(mLock);
    getInputDevicesLocked(outInputDevices);
}

void InputReaderZizhan::getInputDevicesLocked(std::vector<InputDeviceInfo>& outInputDevices) {
...
int32_t InputReaderZizhan::getKeyCodeState(int32_t deviceId, uint32_t sourceMask,
        int32_t keyCode) {
    AutoMutex _l(mLock);

    return getStateLocked(deviceId, sourceMask, keyCode, &InputDeviceZizhan::getKeyCodeState);
}

int32_t InputReaderZizhan::getScanCodeState(int32_t deviceId, uint32_t sourceMask,
        int32_t scanCode) {
    AutoMutex _l(mLock);

    return getStateLocked(deviceId, sourceMask, scanCode, &InputDeviceZizhan::getScanCodeState);
}

int32_t InputReaderZizhan::getSwitchState(int32_t deviceId, uint32_t sourceMask, int32_t switchCode) {
    AutoMutex _l(mLock);

    return getStateLocked(deviceId, sourceMask, switchCode, &InputDeviceZizhan::getSwitchState);
}

int32_t InputReaderZizhan::getStateLocked(int32_t deviceId, uint32_t sourceMask, int32_t code,
...
void InputReaderZizhan::toggleCapsLockState(int32_t deviceId) {
    ssize_t deviceIndex = mDevices.indexOfKey(deviceId);
    if (deviceIndex < 0) {
...
bool InputReaderZizhan::hasKeys(int32_t deviceId, uint32_t sourceMask,
...
bool InputReaderZizhan::markSupportedKeyCodesLocked(int32_t deviceId, uint32_t sourceMask,
        size_t numCodes, const int32_t* keyCodes, uint8_t* outFlags) {
    bool result = false;
    if (deviceId >= 0) {
        ssize_t deviceIndex = mDevices.indexOfKey(deviceId);
...
void InputReaderZizhan::requestRefreshConfiguration(uint32_t changes) {
    AutoMutex _l(mLock);
    ...
    //zizhan
    ALOGD("InputReaderZizhan requestRefreshConfiguration is called");
}

void InputReaderZizhan::vibrate(int32_t deviceId, const nsecs_t* pattern, size_t patternSize,
...
void InputReaderZizhan::cancelVibrate(int32_t deviceId, int32_t token) {
    AutoMutex _l(mLock);
...
bool InputReaderZizhan::isInputDeviceEnabled(int32_t deviceId) {
    AutoMutex _l(mLock);
...
bool InputReaderZizhan::canDispatchToDisplay(int32_t deviceId, int32_t displayId) {
    AutoMutex _l(mLock);
...
void InputReaderZizhan::dump(std::string& dump) {
    AutoMutex _l(mLock);
...
void InputReaderZizhan::monitor() {
...
// --- InputReaderZizhan::ContextImpl ---

InputReaderZizhan::ContextImpl::ContextImpl(InputReaderZizhan* reader) :
        mReader(reader) {
}

void InputReaderZizhan::ContextImpl::updateGlobalMetaState() {
    // lock is already held by the input loop
    mReader->updateGlobalMetaStateLocked();
}

int32_t InputReaderZizhan::ContextImpl::getGlobalMetaState() {
    // lock is already held by the input loop
    return mReader->getGlobalMetaStateLocked();
}

void InputReaderZizhan::ContextImpl::disableVirtualKeysUntil(nsecs_t time) {
    // lock is already held by the input loop
    mReader->disableVirtualKeysUntilLocked(time);
}

bool InputReaderZizhan::ContextImpl::shouldDropVirtualKey(nsecs_t now,
        InputDeviceZizhan* device, int32_t keyCode, int32_t scanCode) {
    // lock is already held by the input loop
    return mReader->shouldDropVirtualKeyLocked(now, device, keyCode, scanCode);
}

void InputReaderZizhan::ContextImpl::fadePointer() {
    // lock is already held by the input loop
    mReader->fadePointerLocked();
}

void InputReaderZizhan::ContextImpl::requestTimeoutAtTime(nsecs_t when) {
    // lock is already held by the input loop
    mReader->requestTimeoutAtTimeLocked(when);
}

int32_t InputReaderZizhan::ContextImpl::bumpGeneration() {
    // lock is already held by the input loop
    return mReader->bumpGenerationLocked();
}

void InputReaderZizhan::ContextImpl::getExternalStylusDevices(std::vector<InputDeviceInfo>& outDevices) {
    // lock is already held by whatever called refreshConfigurationLocked
    mReader->getExternalStylusDevicesLocked(outDevices);
}

void InputReaderZizhan::ContextImpl::dispatchExternalStylusState(const StylusStateZizhan& state) {
    mReader->dispatchExternalStylusState(state);
}

InputReaderPolicyInterface* InputReaderZizhan::ContextImpl::getPolicy() {
    return mReader->mPolicy.get();
}

InputListenerInterface* InputReaderZizhan::ContextImpl::getListener() {
    return mReader->mQueuedListener.get();
}

EventHubInterfaceZizhan* InputReaderZizhan::ContextImpl::getEventHub() {
    return mReader->mEventHub.get();
}

uint32_t InputReaderZizhan::ContextImpl::getNextSequenceNum() {
    return (mReader->mNextSequenceNum)++;
}

// --- InputDeviceZizhan ---

InputDeviceZizhan::InputDeviceZizhan(InputReaderContextZizhan* context, int32_t id, int32_t generation,
...
InputDeviceZizhan::~InputDeviceZizhan() {
...
bool InputDeviceZizhan::isEnabled() {
    return getEventHub()->isDeviceEnabled(mId);
}

void InputDeviceZizhan::setEnabled(bool enabled, nsecs_t when) {
...
void InputDeviceZizhan::dump(std::string& dump) {
    ...
    size_t numMappers = mMappers.size();
    for (size_t i = 0; i < numMappers; i++) {
        InputMapperZizhan* mapper = mMappers[i];
        mapper->dump(dump);
    }
}

void InputDeviceZizhan::addMapper(InputMapperZizhan* mapper) {
    mMappers.push_back(mapper);
}

void InputDeviceZizhan::configure(nsecs_t when, const InputReaderConfiguration* config, uint32_t changes) {
    //zizhan
    ALOGD("device configure is called.");
    mSources = 0;
    ...
    if (!isIgnored()) {
        ...
        if (!changes || (changes & InputReaderConfiguration::CHANGE_KEYBOARD_LAYOUTS)) {
            if (!(mClasses & INPUT_DEVICE_CLASS_VIRTUAL_ZIZHAN)) {
        ...
        for (InputMapperZizhan* mapper : mMappers) {
...
void InputDeviceZizhan::reset(nsecs_t when) {
    for (InputMapperZizhan* mapper : mMappers) {
        mapper->reset(when);
    }
...
void InputDeviceZizhan::process(const RawEventZizhan* rawEvents, size_t count) {
    // Process all of the events in order for each mapper.
    // We cannot simply ask each mapper to process them in bulk because mappers may
    // have side-effects that must be interleaved.  For example, joystick movement events and
    // gamepad button presses are handled by different mappers but they should be dispatched
    // in the order received.
    for (const RawEventZizhan* rawEvent = rawEvents; count != 0; rawEvent++) {
        //zizhan
        ALOGD("[InputDeviceZizhan] Input event: device=%d type=0x%04x code=0x%04x value=0x%08x when=%" PRId64,
...
void InputDeviceZizhan::timeoutExpired(nsecs_t when) {
    for (InputMapperZizhan* mapper : mMappers) {
        mapper->timeoutExpired(when);
    }
}

void InputDeviceZizhan::updateExternalStylusState(const StylusStateZizhan& state) {
    for (InputMapperZizhan* mapper : mMappers) {
        mapper->updateExternalStylusState(state);
    }
}

void InputDeviceZizhan::getDeviceInfo(InputDeviceInfo* outDeviceInfo) {
...
int32_t InputDeviceZizhan::getKeyCodeState(uint32_t sourceMask, int32_t keyCode) {
    return getState(sourceMask, keyCode, & InputMapperZizhan::getKeyCodeState);
}

int32_t InputDeviceZizhan::getScanCodeState(uint32_t sourceMask, int32_t scanCode) {
    return getState(sourceMask, scanCode, & InputMapperZizhan::getScanCodeState);
}

int32_t InputDeviceZizhan::getSwitchState(uint32_t sourceMask, int32_t switchCode) {
    return getState(sourceMask, switchCode, & InputMapperZizhan::getSwitchState);
}

int32_t InputDeviceZizhan::getState(uint32_t sourceMask, int32_t code, GetStateFunc getStateFunc) {
    int32_t result = AKEY_STATE_UNKNOWN;
    for (InputMapperZizhan* mapper : mMappers) {
...
bool InputDeviceZizhan::markSupportedKeyCodes(uint32_t sourceMask, size_t numCodes,
...
void InputDeviceZizhan::vibrate(const nsecs_t* pattern, size_t patternSize, ssize_t repeat,
        int32_t token) {
    for (InputMapperZizhan* mapper : mMappers) {
...
void InputDeviceZizhan::cancelVibrate(int32_t token) {
    for (InputMapperZizhan* mapper : mMappers) {
        mapper->cancelVibrate(token);
    }
}

void InputDeviceZizhan::cancelTouch(nsecs_t when) {
    for (InputMapperZizhan* mapper : mMappers) {
        mapper->cancelTouch(when);
    }
}

int32_t InputDeviceZizhan::getMetaState() {
    int32_t result = 0;
    for (InputMapperZizhan* mapper : mMappers) {
...
void InputDeviceZizhan::updateMetaState(int32_t keyCode) {
    for (InputMapperZizhan* mapper : mMappers) {
        mapper->updateMetaState(keyCode);
    }
}

void InputDeviceZizhan::fadePointer() {
    for (InputMapperZizhan* mapper : mMappers) {
        mapper->fadePointer();
    }
}

void InputDeviceZizhan::bumpGeneration() {
    mGeneration = mContext->bumpGeneration();
}

void InputDeviceZizhan::notifyReset(nsecs_t when) {
    NotifyDeviceResetArgs args(mContext->getNextSequenceNum(), when, mId);
    mContext->getListener()->notifyDeviceReset(&args);
}

std::optional<int32_t> InputDeviceZizhan::getAssociatedDisplay() {
    for (InputMapperZizhan* mapper : mMappers) {
...
// --- CursorButtonAccumulatorZizhan ---

CursorButtonAccumulatorZizhan::CursorButtonAccumulatorZizhan() {
    clearButtons();
}

void CursorButtonAccumulatorZizhan::reset(InputDeviceZizhan* device) {
...
void CursorButtonAccumulatorZizhan::clearButtons() {
    mBtnLeft = 0;
    mBtnRight = 0;
    mBtnMiddle = 0;
    mBtnBack = 0;
...
void CursorButtonAccumulatorZizhan::process(const RawEventZizhan* rawEvent) {
...
uint32_t CursorButtonAccumulatorZizhan::getButtonState() const {
    uint32_t result = 0;
    if (mBtnOk) {
       char mKeyMouseState[PROPERTY_VALUE_MAX] = {0};
        property_get("sys.KeyMouse.mKeyMouseState", mKeyMouseState, "off");
...
// --- CursorMotionAccumulatorZizhan ---

CursorMotionAccumulatorZizhan::CursorMotionAccumulatorZizhan() {
    clearRelativeAxes();
}

void CursorMotionAccumulatorZizhan::reset(InputDeviceZizhan* device) {
    clearRelativeAxes();
}

void CursorMotionAccumulatorZizhan::clearRelativeAxes() {
    mRelX = 0;
    mRelY = 0;
}

void CursorMotionAccumulatorZizhan::process(const RawEventZizhan* rawEvent) {
...
void CursorMotionAccumulatorZizhan::finishSync() {
    clearRelativeAxes();
}


// --- CursorScrollAccumulatorZizhan ---

CursorScrollAccumulatorZizhan::CursorScrollAccumulatorZizhan() :
        mHaveRelWheel(false), mHaveRelHWheel(false) {
    clearRelativeAxes();
}

void CursorScrollAccumulatorZizhan::configure(InputDeviceZizhan* device) {
    mHaveRelWheel = device->getEventHub()->hasRelativeAxis(device->getId(), REL_WHEEL);
    mHaveRelHWheel = device->getEventHub()->hasRelativeAxis(device->getId(), REL_HWHEEL);
}

void CursorScrollAccumulatorZizhan::reset(InputDeviceZizhan* device) {
    clearRelativeAxes();
}

void CursorScrollAccumulatorZizhan::clearRelativeAxes() {
    mRelWheel = 0;
    mRelHWheel = 0;
}

void CursorScrollAccumulatorZizhan::process(const RawEventZizhan* rawEvent) {
    if (rawEvent->type == EV_REL) {
        switch (rawEvent->code) {
        case REL_WHEEL:
...
void CursorScrollAccumulatorZizhan::finishSync() {
    clearRelativeAxes();
}


// --- TouchButtonAccumulatorZizhan ---

TouchButtonAccumulatorZizhan::TouchButtonAccumulatorZizhan() :
        mHaveBtnTouch(false), mHaveStylus(false) {
    clearButtons();
}

void TouchButtonAccumulatorZizhan::configure(InputDeviceZizhan* device) {
...
void TouchButtonAccumulatorZizhan::reset(InputDeviceZizhan* device) {
...
void TouchButtonAccumulatorZizhan::clearButtons() {
    mBtnTouch = 0;
    mBtnStylus = 0;
    mBtnStylus2 = 0;
    mBtnToolFinger = 0;
...
void TouchButtonAccumulatorZizhan::process(const RawEventZizhan* rawEvent) {
    if (rawEvent->type == EV_KEY) {
        switch (rawEvent->code) {
        case BTN_TOUCH:
...
uint32_t TouchButtonAccumulatorZizhan::getButtonState() const {
    uint32_t result = 0;
    if (mBtnStylus) {
...
int32_t TouchButtonAccumulatorZizhan::getToolType() const {
...
bool TouchButtonAccumulatorZizhan::isToolActive() const {
...
bool TouchButtonAccumulatorZizhan::isHovering() const {
    return mHaveBtnTouch && !mBtnTouch;
}

bool TouchButtonAccumulatorZizhan::hasStylus() const {
    return mHaveStylus;
}


// --- RawPointerAxesZizhan ---

RawPointerAxesZizhan::RawPointerAxesZizhan() {
    clear();
}

void RawPointerAxesZizhan::clear() {
    x.clear();
    y.clear();
    pressure.clear();
    touchMajor.clear();
...
// --- RawPointerDataZizhan ---

RawPointerDataZizhan::RawPointerDataZizhan() {
    clear();
}

void RawPointerDataZizhan::clear() {
    pointerCount = 0;
    clearIdBits();
}

void RawPointerDataZizhan::copyFrom(const RawPointerDataZizhan& other) {
...
void RawPointerDataZizhan::getCentroidOfTouchingPointers(float* outX, float* outY) const {
    float x = 0, y = 0;
...
// --- CookedPointerDataZizhan ---

CookedPointerDataZizhan::CookedPointerDataZizhan() {
    clear();
}

void CookedPointerDataZizhan::clear() {
    pointerCount = 0;
    hoveringIdBits.clear();
    touchingIdBits.clear();
}

void CookedPointerDataZizhan::copyFrom(const CookedPointerDataZizhan& other) {
    pointerCount = other.pointerCount;
    hoveringIdBits = other.hoveringIdBits;
    touchingIdBits = other.touchingIdBits;
...
// --- SingleTouchMotionAccumulatorZizhan ---

SingleTouchMotionAccumulatorZizhan::SingleTouchMotionAccumulatorZizhan() {
    clearAbsoluteAxes();
}

void SingleTouchMotionAccumulatorZizhan::reset(InputDeviceZizhan* device) {
...
void SingleTouchMotionAccumulatorZizhan::clearAbsoluteAxes() {
    mAbsX = 0;
    mAbsY = 0;
    mAbsPressure = 0;
    mAbsToolWidth = 0;
...
void SingleTouchMotionAccumulatorZizhan::process(const RawEventZizhan* rawEvent) {
...
// --- MultiTouchMotionAccumulatorZizhan ---

MultiTouchMotionAccumulatorZizhan::MultiTouchMotionAccumulatorZizhan() :
...
void MultiTouchMotionAccumulatorZizhan::configure(InputDeviceZizhan* device,
...
void MultiTouchMotionAccumulatorZizhan::reset(InputDeviceZizhan* device) {
...
void MultiTouchMotionAccumulatorZizhan::clearSlots(int32_t initialSlot) {
    if (mSlots) {
...
void MultiTouchMotionAccumulatorZizhan::process(const RawEventZizhan* rawEvent) {
    if (rawEvent->type == EV_ABS) {
        bool newSlot = false;
        if (mUsingSlotsProtocol) {
...
void MultiTouchMotionAccumulatorZizhan::finishSync() {
    if (!mUsingSlotsProtocol) {
        clearSlots(-1);
    }
}

bool MultiTouchMotionAccumulatorZizhan::hasStylus() const {
    return mHaveStylus;
}


// --- MultiTouchMotionAccumulatorZizhan::Slot ---

MultiTouchMotionAccumulatorZizhan::Slot::Slot() {
    clear();
}

void MultiTouchMotionAccumulatorZizhan::Slot::clear() {
    mInUse = false;
...
int32_t MultiTouchMotionAccumulatorZizhan::Slot::getToolType() const {
...
InputMapperZizhan::InputMapperZizhan(InputDeviceZizhan* device) :
        mDevice(device), mContext(device->getContext()) {
}

InputMapperZizhan::~InputMapperZizhan() {
}

void InputMapperZizhan::populateDeviceInfo(InputDeviceInfo* info) {
    info->addSource(getSources());
}

void InputMapperZizhan::dump(std::string& dump) {
}

void InputMapperZizhan::configure(nsecs_t when,
        const InputReaderConfiguration* config, uint32_t changes) {
}

void InputMapperZizhan::reset(nsecs_t when) {
}

void InputMapperZizhan::timeoutExpired(nsecs_t when) {
}

int32_t InputMapperZizhan::getKeyCodeState(uint32_t sourceMask, int32_t keyCode) {
    return AKEY_STATE_UNKNOWN;
}

int32_t InputMapperZizhan::getScanCodeState(uint32_t sourceMask, int32_t scanCode) {
    return AKEY_STATE_UNKNOWN;
}

int32_t InputMapperZizhan::getSwitchState(uint32_t sourceMask, int32_t switchCode) {
    return AKEY_STATE_UNKNOWN;
}

bool InputMapperZizhan::markSupportedKeyCodes(uint32_t sourceMask, size_t numCodes,
        const int32_t* keyCodes, uint8_t* outFlags) {
    return false;
}

void InputMapperZizhan::vibrate(const nsecs_t* pattern, size_t patternSize, ssize_t repeat,
        int32_t token) {
}

void InputMapperZizhan::cancelVibrate(int32_t token) {
}

void InputMapperZizhan::cancelTouch(nsecs_t when) {
}

int32_t InputMapperZizhan::getMetaState() {
    return 0;
}

void InputMapperZizhan::updateMetaState(int32_t keyCode) {
}

void InputMapperZizhan::updateExternalStylusState(const StylusStateZizhan& state) {

}

void InputMapperZizhan::fadePointer() {
}

status_t InputMapperZizhan::getAbsoluteAxisInfo(int32_t axis, RawAbsoluteAxisInfoZizhan* axisInfo) {
    return getEventHub()->getAbsoluteAxisInfo(getDeviceId(), axis, axisInfo);
}

void InputMapperZizhan::bumpGeneration() {
    mDevice->bumpGeneration();
}

void InputMapperZizhan::dumpRawAbsoluteAxisInfo(std::string& dump,
        const RawAbsoluteAxisInfoZizhan& axis, const char* name) {
    if (axis.valid) {
...
void InputMapperZizhan::dumpStylusState(std::string& dump, const StylusStateZizhan& state) {
...
// --- SwitchInputMapperZizhan ---

SwitchInputMapperZizhan::SwitchInputMapperZizhan(InputDeviceZizhan* device) :
        InputMapperZizhan(device), mSwitchValues(0), mUpdatedSwitchMask(0) {
}

SwitchInputMapperZizhan::~SwitchInputMapperZizhan() {
}

uint32_t SwitchInputMapperZizhan::getSources() {
    return AINPUT_SOURCE_SWITCH;
}

void SwitchInputMapperZizhan::process(const RawEventZizhan* rawEvent) {
    switch (rawEvent->type) {
    case EV_SW:
...
void SwitchInputMapperZizhan::processSwitch(int32_t switchCode, int32_t switchValue) {
...
void SwitchInputMapperZizhan::sync(nsecs_t when) {
...
int32_t SwitchInputMapperZizhan::getSwitchState(uint32_t sourceMask, int32_t switchCode) {
    return getEventHub()->getSwitchState(getDeviceId(), switchCode);
}

void SwitchInputMapperZizhan::dump(std::string& dump) {
    dump += INDENT2 "Switch Input Mapper:\n";
    dump += StringPrintf(INDENT3 "SwitchValues: %x\n", mSwitchValues);
}

// --- VibratorInputMapperZizhan ---

VibratorInputMapperZizhan::VibratorInputMapperZizhan(InputDeviceZizhan* device) :
        InputMapperZizhan(device), mVibrating(false) {
}

VibratorInputMapperZizhan::~VibratorInputMapperZizhan() {
}

uint32_t VibratorInputMapperZizhan::getSources() {
    return 0;
}

void VibratorInputMapperZizhan::populateDeviceInfo(InputDeviceInfo* info) {
    InputMapperZizhan::populateDeviceInfo(info);

    info->setVibrator(true);
}

void VibratorInputMapperZizhan::process(const RawEventZizhan* rawEvent) {
    // TODO: Handle FF_STATUS, although it does not seem to be widely supported.
}

void VibratorInputMapperZizhan::vibrate(const nsecs_t* pattern, size_t patternSize, ssize_t repeat,
        int32_t token) {
#if DEBUG_VIBRATOR
...
void VibratorInputMapperZizhan::cancelVibrate(int32_t token) {
#if DEBUG_VIBRATOR
...
void VibratorInputMapperZizhan::timeoutExpired(nsecs_t when) {
    if (mVibrating) {
...
void VibratorInputMapperZizhan::nextStep() {
    mIndex += 1;
...
void VibratorInputMapperZizhan::stopVibrating() {
    mVibrating = false;
#if DEBUG_VIBRATOR
...
void VibratorInputMapperZizhan::dump(std::string& dump) {
    dump += INDENT2 "Vibrator Input Mapper:\n";
    dump += StringPrintf(INDENT3 "Vibrating: %s\n", toString(mVibrating));
}


// --- KeyboardInputMapperZizhan ---

KeyboardInputMapperZizhan::KeyboardInputMapperZizhan(InputDeviceZizhan* device,
        uint32_t source, int32_t keyboardType) :
        InputMapperZizhan(device), mSource(source), mKeyboardType(keyboardType) {
}

KeyboardInputMapperZizhan::~KeyboardInputMapperZizhan() {
}

uint32_t KeyboardInputMapperZizhan::getSources() {
    return mSource;
}

int32_t KeyboardInputMapperZizhan::getOrientation() {
    if (mViewport) {
...
int32_t KeyboardInputMapperZizhan::getDisplayId() {
    if (mViewport) {
...
void KeyboardInputMapperZizhan::populateDeviceInfo(InputDeviceInfo* info) {
    InputMapperZizhan::populateDeviceInfo(info);
...
void KeyboardInputMapperZizhan::dump(std::string& dump) {
...
void KeyboardInputMapperZizhan::configure(nsecs_t when,
        const InputReaderConfiguration* config, uint32_t changes) {
    InputMapperZizhan::configure(when, config, changes);
...
void KeyboardInputMapperZizhan::configureParameters() {
...
void KeyboardInputMapperZizhan::dumpParameters(std::string& dump) {
...
void KeyboardInputMapperZizhan::reset(nsecs_t when) {
    ...
    InputMapperZizhan::reset(when);
}

void KeyboardInputMapperZizhan::process(const RawEventZizhan* rawEvent) {
    switch (rawEvent->type) {
    case EV_KEY: {
...
bool KeyboardInputMapperZizhan::isKeyboardOrGamepadKey(int32_t scanCode) {
...
bool KeyboardInputMapperZizhan::isMediaKey(int32_t keyCode) {
    switch (keyCode) {
...
void KeyboardInputMapperZizhan::processKey(nsecs_t when, bool down, int32_t scanCode,
        int32_t usageCode) {
    int32_t keyCode;
...
ssize_t KeyboardInputMapperZizhan::findKeyDown(int32_t scanCode) {
...
int32_t KeyboardInputMapperZizhan::getKeyCodeState(uint32_t sourceMask, int32_t keyCode) {
    return getEventHub()->getKeyCodeState(getDeviceId(), keyCode);
}

int32_t KeyboardInputMapperZizhan::getScanCodeState(uint32_t sourceMask, int32_t scanCode) {
    return getEventHub()->getScanCodeState(getDeviceId(), scanCode);
}

bool KeyboardInputMapperZizhan::markSupportedKeyCodes(uint32_t sourceMask, size_t numCodes,
        const int32_t* keyCodes, uint8_t* outFlags) {
    return getEventHub()->markSupportedKeyCodes(getDeviceId(), numCodes, keyCodes, outFlags);
}

int32_t KeyboardInputMapperZizhan::getMetaState() {
    return mMetaState;
}

void KeyboardInputMapperZizhan::updateMetaState(int32_t keyCode) {
    updateMetaStateIfNeeded(keyCode, false);
}

bool KeyboardInputMapperZizhan::updateMetaStateIfNeeded(int32_t keyCode, bool down) {
...
void KeyboardInputMapperZizhan::resetLedState() {
...
void KeyboardInputMapperZizhan::initializeLedState(LedState& ledState, int32_t led) {
    ledState.avail = getEventHub()->hasLed(getDeviceId(), led);
    ledState.on = false;
}

void KeyboardInputMapperZizhan::updateLedState(bool reset) {
...
void KeyboardInputMapperZizhan::updateLedStateForModifier(LedState& ledState,
...
// --- CursorInputMapperZizhan ---

CursorInputMapperZizhan::CursorInputMapperZizhan(InputDeviceZizhan* device) :
        InputMapperZizhan(device) {
}

CursorInputMapperZizhan::~CursorInputMapperZizhan() {
}

uint32_t CursorInputMapperZizhan::getSources() {
    return mSource;
}

void CursorInputMapperZizhan::populateDeviceInfo(InputDeviceInfo* info) {
    InputMapperZizhan::populateDeviceInfo(info);
...
void CursorInputMapperZizhan::dump(std::string& dump) {
...
void CursorInputMapperZizhan::configure(nsecs_t when,
        const InputReaderConfiguration* config, uint32_t changes) {
    InputMapperZizhan::configure(when, config, changes);
...
void CursorInputMapperZizhan::configureParameters() {
    mParameters.mode = Parameters::MODE_POINTER;
...
void CursorInputMapperZizhan::dumpParameters(std::string& dump) {
    dump += INDENT3 "Parameters:\n";
...
void CursorInputMapperZizhan::reset(nsecs_t when) {
    mButtonState = 0;
    mDownTime = 0;
...
void CursorInputMapperZizhan::process(const RawEventZizhan* rawEvent) {
...
void CursorInputMapperZizhan::sync(nsecs_t when) {
...
int32_t CursorInputMapperZizhan::getScanCodeState(uint32_t sourceMask, int32_t scanCode) {
...
void CursorInputMapperZizhan::fadePointer() {
    if (mPointerController != nullptr) {
        mPointerController->fade(PointerControllerInterface::TRANSITION_GRADUAL);
    }
}

std::optional<int32_t> CursorInputMapperZizhan::getAssociatedDisplay() {
...
// --- RotaryEncoderInputMapperZizhan ---

RotaryEncoderInputMapperZizhan::RotaryEncoderInputMapperZizhan(InputDeviceZizhan* device) :
        InputMapperZizhan(device), mOrientation(DISPLAY_ORIENTATION_0) {
    mSource = AINPUT_SOURCE_ROTARY_ENCODER;
}

RotaryEncoderInputMapperZizhan::~RotaryEncoderInputMapperZizhan() {
}

uint32_t RotaryEncoderInputMapperZizhan::getSources() {
    return mSource;
}

void RotaryEncoderInputMapperZizhan::populateDeviceInfo(InputDeviceInfo* info) {
    InputMapperZizhan::populateDeviceInfo(info);
...
void RotaryEncoderInputMapperZizhan::dump(std::string& dump) {
    dump += INDENT2 "Rotary Encoder Input Mapper:\n";
    dump += StringPrintf(INDENT3 "HaveWheel: %s\n",
            toString(mRotaryEncoderScrollAccumulator.haveRelativeVWheel()));
}

void RotaryEncoderInputMapperZizhan::configure(nsecs_t when,
        const InputReaderConfiguration* config, uint32_t changes) {
    InputMapperZizhan::configure(when, config, changes);
    if (!changes) {
...
void RotaryEncoderInputMapperZizhan::reset(nsecs_t when) {
    mRotaryEncoderScrollAccumulator.reset(getDevice());

    InputMapperZizhan::reset(when);
}

void RotaryEncoderInputMapperZizhan::process(const RawEventZizhan* rawEvent) {
...
void RotaryEncoderInputMapperZizhan::sync(nsecs_t when) {
    PointerCoords pointerCoords;
    pointerCoords.clear();
...
KeyMouseInputMapperZizhan::KeyMouseInputMapperZizhan(InputDeviceZizhan* device) : InputMapperZizhan(device) {
}

KeyMouseInputMapperZizhan::~KeyMouseInputMapperZizhan() {
}

uint32_t KeyMouseInputMapperZizhan::getSources() {
    return AINPUT_SOURCE_MOUSE;
}

void KeyMouseInputMapperZizhan::populateDeviceInfo(InputDeviceInfo* info) {
    InputMapperZizhan::populateDeviceInfo(info);
}

void KeyMouseInputMapperZizhan::dump(std::string& dump) {

}

void KeyMouseInputMapperZizhan::configure(nsecs_t when,
        const InputReaderConfiguration* config, uint32_t changes) {
...
void KeyMouseInputMapperZizhan::reset(nsecs_t when) {
    mButtonState = 0;
    mDownTime = 0;
    mCursorButtonAccumulator.reset(getDevice());
...
void KeyMouseInputMapperZizhan::process(const RawEventZizhan* rawEvent) {
...
void KeyMouseInputMapperZizhan::sync(nsecs_t when) {
    int32_t lastButtonState = mButtonState;
    int32_t currentButtonState = mCursorButtonAccumulator.getButtonState();
    mButtonState = currentButtonState;
...
int32_t KeyMouseInputMapperZizhan::getScanCodeState(uint32_t sourceMask, int32_t scanCode) {
    if (scanCode >= BTN_MOUSE && scanCode < BTN_JOYSTICK) {
        return getEventHub()->getScanCodeState(getDeviceId(), scanCode);
    } else {
        return AKEY_STATE_UNKNOWN;
...
void KeyMouseInputMapperZizhan::fadePointer() {

}

// --- TouchInputMapperZizhan ---

TouchInputMapperZizhan::TouchInputMapperZizhan(InputDeviceZizhan* device) :
        InputMapperZizhan(device),
...
TouchInputMapperZizhan::~TouchInputMapperZizhan() {
}

uint32_t TouchInputMapperZizhan::getSources() {
    return mSource;
}

void TouchInputMapperZizhan::populateDeviceInfo(InputDeviceInfo* info) {
    InputMapperZizhan::populateDeviceInfo(info);
...
void TouchInputMapperZizhan::dump(std::string& dump) {
    ...
    for (uint32_t i = 0; i < mLastRawState.rawPointerData.pointerCount; i++) {
        const RawPointerDataZizhan::Pointer& pointer = mLastRawState.rawPointerData.pointers[i];
...
const char* TouchInputMapperZizhan::modeToString(DeviceMode deviceMode) {
    switch (deviceMode) {
...
void TouchInputMapperZizhan::configure(nsecs_t when,
        const InputReaderConfiguration* config, uint32_t changes) {
    InputMapperZizhan::configure(when, config, changes);
    //zizhan
    ALOGD("TouchInputMapper configure is called.");
    ...
    if (!changes || (changes & (InputReaderConfiguration::CHANGE_DISPLAY_INFO
        ...
        ALOGD("after the configuraSurface, the mDeviceMode is [%d].", mDeviceMode);
    }
...
void TouchInputMapperZizhan::resolveExternalStylusPresence() {
...
void TouchInputMapperZizhan::configureParameters() {
...
void TouchInputMapperZizhan::dumpParameters(std::string& dump) {
    dump += INDENT3 "Parameters:\n";
...
void TouchInputMapperZizhan::configureRawPointerAxes() {
    mRawPointerAxes.clear();
}

void TouchInputMapperZizhan::dumpRawPointerAxes(std::string& dump) {
...
bool TouchInputMapperZizhan::hasExternalStylus() const {
 ...
 */
std::optional<DisplayViewport> TouchInputMapperZizhan::findViewport() {
    if (mParameters.hasAssociatedDisplay) {
...
void TouchInputMapperZizhan::configureSurface(nsecs_t when, bool* outResetNeeded) {
    //zizhan
    int32_t oldDeviceMode = mDeviceMode;
    ALOGD("configuraSurface is called, the oldDeviceMode is [%d].", oldDeviceMode);
...
void TouchInputMapperZizhan::dumpSurface(std::string& dump) {
...
void TouchInputMapperZizhan::configureVirtualKeys() {
...
void TouchInputMapperZizhan::dumpVirtualKeys(std::string& dump) {
...
void TouchInputMapperZizhan::parseCalibration() {
...
void TouchInputMapperZizhan::resolveCalibration() {
    // Size
...
void TouchInputMapperZizhan::dumpCalibration(std::string& dump) {
...
void TouchInputMapperZizhan::dumpAffineTransformation(std::string& dump) {
...
void TouchInputMapperZizhan::updateAffineTransformation() {
    mAffineTransform = getPolicy()->getTouchAffineTransformation(mDevice->getDescriptor(),
            mSurfaceOrientation);
}

void TouchInputMapperZizhan::reset(nsecs_t when) {
    mCursorButtonAccumulator.reset(getDevice());
    mCursorScrollAccumulator.reset(getDevice());
...
void TouchInputMapperZizhan::resetExternalStylus() {
...
void TouchInputMapperZizhan::clearStylusDataPendingFlags() {
    mExternalStylusDataPending = false;
    mExternalStylusFusionTimeout = LLONG_MAX;
}

void TouchInputMapperZizhan::reportEventForStatistics(nsecs_t evdevTime) {
...
void TouchInputMapperZizhan::process(const RawEventZizhan* rawEvent) {
    //zizhan
    ALOGD("[TouchInputMapperZizhan], process is called.");
    mCursorButtonAccumulator.process(rawEvent);
    mCursorScrollAccumulator.process(rawEvent);
    mTouchButtonAccumulator.process(rawEvent);
    ALOGD("[TouchInputMapperZizhan], before sync.");

    if (rawEvent->type == EV_SYN && rawEvent->code == SYN_REPORT) {
        reportEventForStatistics(rawEvent->when);
        sync(rawEvent->when);
    }
    ALOGD("[TouchInputMapperZizhan], after sync.");
}

void TouchInputMapperZizhan::sync(nsecs_t when) {
...
void TouchInputMapperZizhan::processRawTouches(bool timeout) {
    ALOGD("[TouchInputMapperZizhan] processRawTouches is called.");
    //zizhan
    if (mDeviceMode == DEVICE_MODE_DISABLED) {
        ...
        //mRawStatesPending.clear();
        //return;
        ALOGD("[TouchInputMapperZizhan] DEVICE_MODE_DISABLED is TRUE!!! Do nothing!");
    }
    ALOGD("[TouchInputMapperZizhan] DEVICE_MODE_DISABLED not return.");
...
void TouchInputMapperZizhan::cookAndDispatch(nsecs_t when) {
    ALOGD("[TouchInputMapperZizhan] cookAndDispatch is called.");
    ...
    ALOGD("[TouchInputMapperZizhan] before consumeRawTouches, currentPointerCount is [%d].", currentPointerCount);
    ...
    ALOGD("[TouchInputMapperZizhan] after, currentPointerCount is [%d].", currentPointerCount);
    ...
    if (mDeviceMode == DEVICE_MODE_POINTER) {
        for (BitSet32 idBits(mCurrentRawState.rawPointerData.touchingIdBits);
                !idBits.isEmpty(); ) {
            uint32_t id = idBits.clearFirstMarkedBit();
            const RawPointerDataZizhan::Pointer& pointer =
...
void TouchInputMapperZizhan::applyExternalStylusButtonState(nsecs_t when) {
    if (mDeviceMode == DEVICE_MODE_DIRECT && hasExternalStylus() && mExternalStylusId != -1) {
        mCurrentRawState.buttonState |= mExternalStylusState.buttons;
    }
}

void TouchInputMapperZizhan::applyExternalStylusTouchState(nsecs_t when) {
    CookedPointerDataZizhan& currentPointerData = mCurrentCookedState.cookedPointerData;
    const CookedPointerDataZizhan& lastPointerData = mLastCookedState.cookedPointerData;
...
bool TouchInputMapperZizhan::assignExternalStylusId(const RawState& state, bool timeout) {
...
void TouchInputMapperZizhan::timeoutExpired(nsecs_t when) {
    if (mDeviceMode == DEVICE_MODE_POINTER) {
...
void TouchInputMapperZizhan::updateExternalStylusState(const StylusStateZizhan& state) {
...
bool TouchInputMapperZizhan::consumeRawTouches(nsecs_t when, uint32_t policyFlags) {
    ...
    if (mLastRawState.rawPointerData.touchingIdBits.isEmpty()
        ...
        uint32_t id = mCurrentRawState.rawPointerData.touchingIdBits.firstMarkedBit();
        const RawPointerDataZizhan::Pointer& pointer = mCurrentRawState.rawPointerData.pointerForId(id);
...
void TouchInputMapperZizhan::dispatchVirtualKey(nsecs_t when, uint32_t policyFlags,
...
void TouchInputMapperZizhan::abortTouches(nsecs_t when, uint32_t policyFlags) {
...
void TouchInputMapperZizhan::dispatchTouches(nsecs_t when, uint32_t policyFlags) {
    BitSet32 currentIdBits = mCurrentCookedState.cookedPointerData.touchingIdBits;
...
void TouchInputMapperZizhan::dispatchHoverExit(nsecs_t when, uint32_t policyFlags) {
    if (mSentHoverEnter &&
...
void TouchInputMapperZizhan::dispatchHoverEnterAndMove(nsecs_t when, uint32_t policyFlags) {
...
void TouchInputMapperZizhan::dispatchButtonRelease(nsecs_t when, uint32_t policyFlags) {
...
void TouchInputMapperZizhan::dispatchButtonPress(nsecs_t when, uint32_t policyFlags) {
...
const BitSet32& TouchInputMapperZizhan::findActiveIdBits(const CookedPointerDataZizhan& cookedPointerData) {
...
void TouchInputMapperZizhan::cookPointerData() {
    ALOGD("[TouchInputMapperZizhan] cookPointerData is called.");
    uint32_t currentPointerCount = mCurrentRawState.rawPointerData.pointerCount;
    ALOGD("[TouchInputMapperZizhan] currentPointerCount is [%d].", currentPointerCount);
    ...
    for (uint32_t i = 0; i < currentPointerCount; i++) {
        const RawPointerDataZizhan::Pointer& in = mCurrentRawState.rawPointerData.pointers[i];
        ...
        //zizhan
        ALOGD("[InputReaderZizhan] After process, the Android Event is [x=%f, y=%f, pressure=%f, size=%f, touchMajor=%f, touchMinor=%f, orientation=%f, tilt=%f, distance=%f, left=%f, top=%f, right=%f, bottom=%f]\n", 
            x, y, pressure, size, touchMajor, touchMinor, orientation, tilt, distance, left, top, right, bottom);
...
void TouchInputMapperZizhan::dispatchPointerUsage(nsecs_t when, uint32_t policyFlags,
...
void TouchInputMapperZizhan::abortPointerUsage(nsecs_t when, uint32_t policyFlags) {
    switch (mPointerUsage) {
    case POINTER_USAGE_GESTURES:
...
void TouchInputMapperZizhan::dispatchPointerGestures(nsecs_t when, uint32_t policyFlags,
        bool isTimeout) {
...
void TouchInputMapperZizhan::abortPointerGestures(nsecs_t when, uint32_t policyFlags) {
...
bool TouchInputMapperZizhan::preparePointerGestures(nsecs_t when,
    ...
    *outFinishPreviousGesture = false;
    ...
    if (isQuietTime) {
        ...
        if (activeTouchId >= 0 && mLastCookedState.fingerIdBits.hasBit(activeTouchId)) {
            const RawPointerDataZizhan::Pointer& currentPointer =
                    mCurrentRawState.rawPointerData.pointerForId(activeTouchId);
            const RawPointerDataZizhan::Pointer& lastPointer =
        ...
        for (BitSet32 idBits(commonIdBits); !idBits.isEmpty(); ) {
            ...
            uint32_t id = idBits.clearFirstMarkedBit();
            const RawPointerDataZizhan::Pointer& cpd = mCurrentRawState.rawPointerData.pointerForId(id);
            const RawPointerDataZizhan::Pointer& lpd = mLastRawState.rawPointerData.pointerForId(id);
            ...
            if (distOverThreshold >= 2) {
                if (currentFingerCount > 2) {
                    ...
                    *outCancelPreviousGesture = true;
                    ...
                    const RawPointerDataZizhan::Pointer& p1 =
                            mCurrentRawState.rawPointerData.pointerForId(id1);
                    const RawPointerDataZizhan::Pointer& p2 =
...
void TouchInputMapperZizhan::dispatchPointerStylus(nsecs_t when, uint32_t policyFlags) {
...
void TouchInputMapperZizhan::abortPointerStylus(nsecs_t when, uint32_t policyFlags) {
    abortPointerSimple(when, policyFlags);
}

void TouchInputMapperZizhan::dispatchPointerMouse(nsecs_t when, uint32_t policyFlags) {
    mPointerSimple.currentCoords.clear();
    mPointerSimple.currentProperties.clear();
...
void TouchInputMapperZizhan::abortPointerMouse(nsecs_t when, uint32_t policyFlags) {
    abortPointerSimple(when, policyFlags);

    mPointerVelocityControl.reset();
}

void TouchInputMapperZizhan::dispatchPointerSimple(nsecs_t when, uint32_t policyFlags,
...
void TouchInputMapperZizhan::abortPointerSimple(nsecs_t when, uint32_t policyFlags) {
    mPointerSimple.currentCoords.clear();
    mPointerSimple.currentProperties.clear();
...
void TouchInputMapperZizhan::dispatchMotion(nsecs_t when, uint32_t policyFlags, uint32_t source,
...
bool TouchInputMapperZizhan::updateMovedPointers(const PointerProperties* inProperties,
...
void TouchInputMapperZizhan::fadePointer() {
    if (mPointerController != nullptr) {
        mPointerController->fade(PointerControllerInterface::TRANSITION_GRADUAL);
    }
}

void TouchInputMapperZizhan::cancelTouch(nsecs_t when) {
    abortPointerUsage(when, 0 /*policyFlags*/);
    abortTouches(when, 0 /* policyFlags*/);
}

bool TouchInputMapperZizhan::isPointInsideSurface(int32_t x, int32_t y) {
...
const TouchInputMapperZizhan::VirtualKey* TouchInputMapperZizhan::findVirtualKeyHit(int32_t x, int32_t y) {
...
void TouchInputMapperZizhan::assignPointerIds(const RawState* last, RawState* current) {
...
int32_t TouchInputMapperZizhan::getKeyCodeState(uint32_t sourceMask, int32_t keyCode) {
...
int32_t TouchInputMapperZizhan::getScanCodeState(uint32_t sourceMask, int32_t scanCode) {
...
bool TouchInputMapperZizhan::markSupportedKeyCodes(uint32_t sourceMask, size_t numCodes,
        const int32_t* keyCodes, uint8_t* outFlags) {
    for (const VirtualKey& virtualKey : mVirtualKeys) {
...
std::optional<int32_t> TouchInputMapperZizhan::getAssociatedDisplay() {
    if (mParameters.hasAssociatedDisplay) {
        if (mDeviceMode == DEVICE_MODE_POINTER) {
            return std::make_optional(mPointerController->getDisplayId());
        } else {
...
// --- SingleTouchInputMapperZizhan ---

SingleTouchInputMapperZizhan::SingleTouchInputMapperZizhan(InputDeviceZizhan* device) :
        TouchInputMapperZizhan(device) {
}

SingleTouchInputMapperZizhan::~SingleTouchInputMapperZizhan() {
}

void SingleTouchInputMapperZizhan::reset(nsecs_t when) {
    mSingleTouchMotionAccumulator.reset(getDevice());

    TouchInputMapperZizhan::reset(when);
}

void SingleTouchInputMapperZizhan::process(const RawEventZizhan* rawEvent) {
    TouchInputMapperZizhan::process(rawEvent);

    mSingleTouchMotionAccumulator.process(rawEvent);
}

void SingleTouchInputMapperZizhan::syncTouch(nsecs_t when, RawState* outState) {
    if (mTouchButtonAccumulator.isToolActive()) {
        ...
        RawPointerDataZizhan::Pointer& outPointer = outState->rawPointerData.pointers[0];
        outPointer.id = 0;
...
void SingleTouchInputMapperZizhan::configureRawPointerAxes() {
    TouchInputMapperZizhan::configureRawPointerAxes();
...
bool SingleTouchInputMapperZizhan::hasStylus() const {
    return mTouchButtonAccumulator.hasStylus();
}


// --- MultiTouchInputMapperZizhan ---

MultiTouchInputMapperZizhan::MultiTouchInputMapperZizhan(InputDeviceZizhan* device) :
        TouchInputMapperZizhan(device) {
}

MultiTouchInputMapperZizhan::~MultiTouchInputMapperZizhan() {
}

void MultiTouchInputMapperZizhan::reset(nsecs_t when) {
...
void MultiTouchInputMapperZizhan::process(const RawEventZizhan* rawEvent) {
    TouchInputMapperZizhan::process(rawEvent);

    mMultiTouchMotionAccumulator.process(rawEvent);
}

void MultiTouchInputMapperZizhan::syncTouch(nsecs_t when, RawState* outState) {
    ...
    for (size_t inIndex = 0; inIndex < inCount; inIndex++) {
        const MultiTouchMotionAccumulatorZizhan::Slot* inSlot =
        ...
        RawPointerDataZizhan::Pointer& outPointer = outState->rawPointerData.pointers[outCount];
...
void MultiTouchInputMapperZizhan::configureRawPointerAxes() {
    TouchInputMapperZizhan::configureRawPointerAxes();
...
bool MultiTouchInputMapperZizhan::hasStylus() const {
    return mMultiTouchMotionAccumulator.hasStylus()
            || mTouchButtonAccumulator.hasStylus();
}

// --- ExternalStylusInputMapperZizhan

ExternalStylusInputMapperZizhan::ExternalStylusInputMapperZizhan(InputDeviceZizhan* device) :
    InputMapperZizhan(device) {

}

uint32_t ExternalStylusInputMapperZizhan::getSources() {
    return AINPUT_SOURCE_STYLUS;
}

void ExternalStylusInputMapperZizhan::populateDeviceInfo(InputDeviceInfo* info) {
    InputMapperZizhan::populateDeviceInfo(info);
    info->addMotionRange(AMOTION_EVENT_AXIS_PRESSURE, AINPUT_SOURCE_STYLUS,
            0.0f, 1.0f, 0.0f, 0.0f, 0.0f);
}

void ExternalStylusInputMapperZizhan::dump(std::string& dump) {
...
void ExternalStylusInputMapperZizhan::configure(nsecs_t when,
        const InputReaderConfiguration* config, uint32_t changes) {
    getAbsoluteAxisInfo(ABS_PRESSURE, &mRawPressureAxis);
    mTouchButtonAccumulator.configure(getDevice());
}

void ExternalStylusInputMapperZizhan::reset(nsecs_t when) {
    InputDeviceZizhan* device = getDevice();
    mSingleTouchMotionAccumulator.reset(device);
    mTouchButtonAccumulator.reset(device);
    InputMapperZizhan::reset(when);
}

void ExternalStylusInputMapperZizhan::process(const RawEventZizhan* rawEvent) {
    mSingleTouchMotionAccumulator.process(rawEvent);
    mTouchButtonAccumulator.process(rawEvent);
...
void ExternalStylusInputMapperZizhan::sync(nsecs_t when) {
    mStylusState.clear();
...
// --- JoystickInputMapperZizhan ---

JoystickInputMapperZizhan::JoystickInputMapperZizhan(InputDeviceZizhan* device) :
        InputMapperZizhan(device) {
}

JoystickInputMapperZizhan::~JoystickInputMapperZizhan() {
}

uint32_t JoystickInputMapperZizhan::getSources() {
    return AINPUT_SOURCE_JOYSTICK;
}

void JoystickInputMapperZizhan::populateDeviceInfo(InputDeviceInfo* info) {
    InputMapperZizhan::populateDeviceInfo(info);
...
void JoystickInputMapperZizhan::addMotionRange(int32_t axisId, const Axis& axis,
        InputDeviceInfo* info) {
...
int32_t JoystickInputMapperZizhan::getCompatAxis(int32_t axis) {
    switch(axis) {
...
void JoystickInputMapperZizhan::dump(std::string& dump) {
...
void JoystickInputMapperZizhan::configure(nsecs_t when,
        const InputReaderConfiguration* config, uint32_t changes) {
    InputMapperZizhan::configure(when, config, changes);
    ...
    if (!changes) { // first time only
        ...
        for (int32_t abs = 0; abs <= ABS_MAX; abs++) {
            if (!(getAbsAxisUsage(abs, getDevice()->getClasses())
                    & INPUT_DEVICE_CLASS_JOYSTICK_ZIZHAN)) {
                continue; // axis must be claimed by a different device
            }

            RawAbsoluteAxisInfoZizhan rawAxisInfo;
...
bool JoystickInputMapperZizhan::haveAxis(int32_t axisId) {
    size_t numAxes = mAxes.size();
    for (size_t i = 0; i < numAxes; i++) {
        const Axis& axis = mAxes.valueAt(i);
...
void JoystickInputMapperZizhan::pruneAxes(bool ignoreExplicitlyMappedAxes) {
    size_t i = mAxes.size();
...
bool JoystickInputMapperZizhan::isCenteredAxis(int32_t axis) {
    switch (axis) {
...
void JoystickInputMapperZizhan::reset(nsecs_t when) {
    // Recenter all axes.
    size_t numAxes = mAxes.size();
    for (size_t i = 0; i < numAxes; i++) {
        Axis& axis = mAxes.editValueAt(i);
...
void JoystickInputMapperZizhan::process(const RawEventZizhan* rawEvent) {
    switch (rawEvent->type) {
    case EV_ABS: {
...
void JoystickInputMapperZizhan::sync(nsecs_t when, bool force) {
    if (!filterAxes(force)) {
        return;
    }
...
void JoystickInputMapperZizhan::setPointerCoordsAxisValue(PointerCoords* pointerCoords,
...
bool JoystickInputMapperZizhan::filterAxes(bool force) {
...
bool JoystickInputMapperZizhan::hasValueChangedSignificantly(
...
bool JoystickInputMapperZizhan::hasMovedNearerToValueWithinFilteredRange(
