...
#define LOG_TAG "EventHubZizhan"

// #define LOG_NDEBUG 0

#include <inputzizhan/EventHubZizhan.h>
#include <inputzizhan/LibZizhan.h>
...
uint32_t getAbsAxisUsage(int32_t axis, uint32_t deviceClasses) {
    ...
    if (deviceClasses & INPUT_DEVICE_CLASS_TOUCH_ZIZHAN) {
        switch (axis) {
        case ABS_X:
        case ABS_Y:
        case ABS_PRESSURE:
        ...
        case ABS_MT_DISTANCE:
            return INPUT_DEVICE_CLASS_TOUCH_ZIZHAN;
        }
    }

    // External stylus gets the pressure axis
    if (deviceClasses & INPUT_DEVICE_CLASS_EXTERNAL_STYLUS_ZIZHAN) {
        if (axis == ABS_PRESSURE) {
            return INPUT_DEVICE_CLASS_EXTERNAL_STYLUS_ZIZHAN;
        }
    }

    // Joystick devices get the rest.
    return deviceClasses & INPUT_DEVICE_CLASS_JOYSTICK_ZIZHAN;
}

// --- EventHubZizhan::Device ---

EventHubZizhan::Device::Device(int fd, int32_t id, const std::string& path,
...
EventHubZizhan::Device::~Device() {
    close();
    delete configuration;
}

void EventHubZizhan::Device::close() {
    if (fd >= 0) {
        ::close(fd);
        fd = -1;
    }
...
status_t EventHubZizhan::Device::enable() {
...
status_t EventHubZizhan::Device::disable() {
    close();
    enabled = false;
    return OK;
}

bool EventHubZizhan::Device::hasValidFd() {
    return !isVirtual && enabled;
}

// --- EventHub ---

const int EventHubZizhan::EPOLL_MAX_EVENTS;

EventHubZizhan::EventHubZizhan(void) :
...
InputDeviceIdentifier EventHubZizhan::getDeviceIdentifier(int32_t deviceId) const {
    AutoMutex _l(mLock);
...
uint32_t EventHubZizhan::getDeviceClasses(int32_t deviceId) const {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
...
int32_t EventHubZizhan::getDeviceControllerNumber(int32_t deviceId) const {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
    if (device == nullptr) return 0;
...
void EventHubZizhan::getConfiguration(int32_t deviceId, PropertyMap* outConfiguration) const {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
...
status_t EventHubZizhan::getAbsoluteAxisInfo(int32_t deviceId, int axis,
        RawAbsoluteAxisInfoZizhan* outAxisInfo) const {
    outAxisInfo->clear();
...
bool EventHubZizhan::hasRelativeAxis(int32_t deviceId, int axis) const {
...
bool EventHubZizhan::hasInputProperty(int32_t deviceId, int property) const {
...
int32_t EventHubZizhan::getScanCodeState(int32_t deviceId, int32_t scanCode) const {
...
int32_t EventHubZizhan::getKeyCodeState(int32_t deviceId, int32_t keyCode) const {
    AutoMutex _l(mLock);
...
int32_t EventHubZizhan::getSwitchState(int32_t deviceId, int32_t sw) const {
    if (sw >= 0 && sw <= SW_MAX) {
        AutoMutex _l(mLock);
...
status_t EventHubZizhan::getAbsoluteAxisValue(int32_t deviceId, int32_t axis, int32_t* outValue) const {
    *outValue = 0;
...
bool EventHubZizhan::markSupportedKeyCodes(int32_t deviceId, size_t numCodes,
...
status_t EventHubZizhan::mapKey(int32_t deviceId,
...
status_t EventHubZizhan::mapAxis(int32_t deviceId, int32_t scanCode, AxisInfo* outAxisInfo) const {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
...
void EventHubZizhan::setExcludedDevices(const std::vector<std::string>& devices) {
    AutoMutex _l(mLock);

    mExcludedDevices = devices;
}

bool EventHubZizhan::hasScanCode(int32_t deviceId, int32_t scanCode) const {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
...
bool EventHubZizhan::hasLed(int32_t deviceId, int32_t led) const {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
    int32_t sc;
...
void EventHubZizhan::setLedState(int32_t deviceId, int32_t led, bool on) {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
    setLedStateLocked(device, led, on);
}

void EventHubZizhan::setLedStateLocked(Device* device, int32_t led, bool on) {
    int32_t sc;
...
void EventHubZizhan::getVirtualKeyDefinitions(int32_t deviceId,
...
sp<KeyCharacterMap> EventHubZizhan::getKeyCharacterMap(int32_t deviceId) const {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
    if (device) {
...
bool EventHubZizhan::setKeyboardLayoutOverlay(int32_t deviceId,
...
void EventHubZizhan::assignDescriptorLocked(InputDeviceIdentifier& identifier) {
...
void EventHubZizhan::vibrate(int32_t deviceId, nsecs_t duration) {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
...
void EventHubZizhan::cancelVibrate(int32_t deviceId) {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
    if (device && device->hasValidFd()) {
...
EventHubZizhan::Device* EventHubZizhan::getDeviceByDescriptorLocked(const std::string& descriptor) const {
...
EventHubZizhan::Device* EventHubZizhan::getDeviceLocked(int32_t deviceId) const {
...
EventHubZizhan::Device* EventHubZizhan::getDeviceByPathLocked(const char* devicePath) const {
 ...
 * devices are ignored.
 */
EventHubZizhan::Device* EventHubZizhan::getDeviceByFdLocked(int fd) const {
    for (size_t i = 0; i < mDevices.size(); i++) {
        Device* device = mDevices.valueAt(i);
        if (device->fd == fd) {
...
//size_t EventHubZizhan::getEvents(int timeoutMillis, RawEventZizhan* buffer, size_t bufferSize){
//    ALOG_ASSERT(bufferSize >= 1);
//    AutoMutex _l(mLock);

//    compat_event_t readBuffer[bufferSize];
//    fetchEvent(readBuffer);

//    compat_event_t& iev = readBuffer[0];
//    RawEventZizhan* event = buffer;

//    event->deviceId = getDeviceIdLocked();

//    event->when = processEventTimestamp(iev);
//    event->type = iev.type;
//    event->code = iev.code;
//    event->value = iev.value;
//    ALOGD("[zizhan] aftre process, the raw event is [when=%ld, deviceId=%d, type=%d, code=%d, value=%d]\n",
//            (long) event->when, event->deviceId, event->type, event->code, event->value);
//}
...
std::vector<TouchVideoFrame> EventHubZizhan::getVideoFrames(int32_t deviceId) {
    AutoMutex _l(mLock);
...
void EventHubZizhan::scanDevicesLocked() {
...
status_t EventHubZizhan::registerFdForEpoll(int fd) {
    // TODO(b/121395353) - consider adding EPOLLRDHUP
    struct epoll_event eventItem = {};
...
status_t EventHubZizhan::unregisterFdFromEpoll(int fd) {
...
status_t EventHubZizhan::registerDeviceForEpollLocked(Device* device) {
    if (device == nullptr) {
        if (DEBUG) {
...
void EventHubZizhan::registerVideoDeviceForEpollLocked(const TouchVideoDeviceZizhan& videoDevice) {
...
status_t EventHubZizhan::unregisterDeviceFromEpollLocked(Device* device) {
...
void EventHubZizhan::unregisterVideoDeviceFromEpollLocked(const TouchVideoDeviceZizhan& videoDevice) {
...
status_t EventHubZizhan::openDeviceLocked(const char* devicePath) {
    char buffer[80];
    ...
    if (haveKeyboardKeys || haveGamepadButtons) {
        device->classes |= INPUT_DEVICE_CLASS_KEYBOARD_ZIZHAN;
        ...
        if (strcmp(targetProduct, "box") == 0) {
           device->classes |= INPUT_DEVICE_CLASS_KEYMOUSE_ZIZHAN;
        }
    }
    ...
    if (test_bit(BTN_MOUSE, device->keyBitmask)
        ...
        device->classes |= INPUT_DEVICE_CLASS_CURSOR_ZIZHAN;
    }
    ...
    if (device->configuration &&
        device->configuration->tryGetProperty(String8("device.type"), deviceType)) {
            if (!deviceType.compare(String8("rotaryEncoder"))) {
                device->classes |= INPUT_DEVICE_CLASS_ROTARY_ENCODER_ZIZHAN;
            }
    }
    ...
    if (test_bit(ABS_MT_POSITION_X, device->absBitmask)
        ...
        if (test_bit(BTN_TOUCH, device->keyBitmask) || !haveGamepadButtons) {
            device->classes |= INPUT_DEVICE_CLASS_TOUCH_ZIZHAN | INPUT_DEVICE_CLASS_TOUCH_MT_ZIZHAN;
        }
    // Is this an old style single-touch driver?
    } else if (test_bit(BTN_TOUCH, device->keyBitmask)
            && test_bit(ABS_X, device->absBitmask)
            && test_bit(ABS_Y, device->absBitmask)) {
        device->classes |= INPUT_DEVICE_CLASS_TOUCH_ZIZHAN;
    // Is this a BT stylus?
    } else if ((test_bit(ABS_PRESSURE, device->absBitmask) ||
                test_bit(BTN_TOUCH, device->keyBitmask))
            && !test_bit(ABS_X, device->absBitmask)
            && !test_bit(ABS_Y, device->absBitmask)) {
        device->classes |= INPUT_DEVICE_CLASS_EXTERNAL_STYLUS_ZIZHAN;
    ...
    if (haveGamepadButtons) {
        uint32_t assumedClasses = device->classes | INPUT_DEVICE_CLASS_JOYSTICK_ZIZHAN;
        for (int i = 0; i <= ABS_MAX; i++) {
            if (test_bit(i, device->absBitmask)
                    && (getAbsAxisUsage(i, assumedClasses) & INPUT_DEVICE_CLASS_JOYSTICK_ZIZHAN)) {
    ...
    for (int i = 0; i <= SW_MAX; i++) {
        if (test_bit(i, device->swBitmask)) {
            device->classes |= INPUT_DEVICE_CLASS_SWITCH_ZIZHAN;
            break;
        }
    }
    ...
    if (test_bit(FF_RUMBLE, device->ffBitmask)) {
        device->classes |= INPUT_DEVICE_CLASS_VIBRATOR_ZIZHAN;
    }

    // Configure virtual keys.
    if ((device->classes & INPUT_DEVICE_CLASS_TOUCH_ZIZHAN)) {
    ...
    if (device->classes & (INPUT_DEVICE_CLASS_KEYBOARD_ZIZHAN | INPUT_DEVICE_CLASS_JOYSTICK_ZIZHAN)) {
        // Load the keymap for the device.
        keyMapStatus = loadKeyMapLocked(device);
    }

    // Configure the keyboard, gamepad or virtual keyboard.
    if (device->classes & INPUT_DEVICE_CLASS_KEYBOARD_ZIZHAN) {
        ...
        if (hasKeycodeLocked(device, AKEYCODE_Q)) {
            device->classes |= INPUT_DEVICE_CLASS_ALPHAKEY_ZIZHAN;
        }
        ...
        if (hasKeycodeLocked(device, AKEYCODE_DPAD_UP) &&
            ...
            device->classes |= INPUT_DEVICE_CLASS_DPAD_ZIZHAN;
        }

        // See if this device has a gamepad.
        for (size_t i = 0; i < sizeof(GAMEPAD_KEYCODES)/sizeof(GAMEPAD_KEYCODES[0]); i++) {
            if (hasKeycodeLocked(device, GAMEPAD_KEYCODES[i])) {
                device->classes |= INPUT_DEVICE_CLASS_GAMEPAD_ZIZHAN;
                break;
            }
        }
    }
    ...
    if (deviceHasMicLocked(device)) {
        device->classes |= INPUT_DEVICE_CLASS_MIC_ZIZHAN;
    }

    // Determine whether the device is external or internal.
    if (isExternalDeviceLocked(device)) {
        device->classes |= INPUT_DEVICE_CLASS_EXTERNAL_ZIZHAN;
    }

    if (device->classes & (INPUT_DEVICE_CLASS_JOYSTICK_ZIZHAN | INPUT_DEVICE_CLASS_DPAD_ZIZHAN)
            && device->classes & INPUT_DEVICE_CLASS_GAMEPAD_ZIZHAN) {
    ...
    for (std::unique_ptr<TouchVideoDeviceZizhan>& videoDevice : mUnattachedVideoDevices) {
    ...
    mUnattachedVideoDevices.erase(std::remove_if(mUnattachedVideoDevices.begin(),
            ...
            [](const std::unique_ptr<TouchVideoDeviceZizhan>& videoDevice){
...
void EventHubZizhan::configureFd(Device* device) {
...
void EventHubZizhan::openVideoDeviceLocked(const std::string& devicePath) {
    std::unique_ptr<TouchVideoDeviceZizhan> videoDevice = TouchVideoDeviceZizhan::create(devicePath);
    if (!videoDevice) {
...
bool EventHubZizhan::isDeviceEnabled(int32_t deviceId) {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
    if (device == nullptr) {
...
status_t EventHubZizhan::enableDevice(int32_t deviceId) {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
    if (device == nullptr) {
        ALOGE("Invalid device id=%" PRId32 " provided to %s", deviceId, __func__);
...
status_t EventHubZizhan::disableDevice(int32_t deviceId) {
    AutoMutex _l(mLock);
    Device* device = getDeviceLocked(deviceId);
    if (device == nullptr) {
        ALOGE("Invalid device id=%" PRId32 " provided to %s", deviceId, __func__);
...
void EventHubZizhan::createVirtualKeyboardLocked() {
    InputDeviceIdentifier identifier;
    ...
    Device* device = new Device(-1, ReservedInputDeviceId::VIRTUAL_KEYBOARD_ID, "<virtual>",
            identifier);
    device->classes = INPUT_DEVICE_CLASS_KEYBOARD_ZIZHAN
            | INPUT_DEVICE_CLASS_ALPHAKEY_ZIZHAN
            | INPUT_DEVICE_CLASS_DPAD_ZIZHAN
            | INPUT_DEVICE_CLASS_VIRTUAL_ZIZHAN;
    loadKeyMapLocked(device);
    addDeviceLocked(device);
}

void EventHubZizhan::addDeviceLocked(Device* device) {
    mDevices.add(device->id, device);
    device->next = mOpeningDevices;
    mOpeningDevices = device;
}

void EventHubZizhan::loadConfigurationLocked(Device* device) {
...
bool EventHubZizhan::loadVirtualKeyMapLocked(Device* device) {
...
status_t EventHubZizhan::loadKeyMapLocked(Device* device) {
    return device->keyMap.load(device->identifier, device->configuration);
}

bool EventHubZizhan::isExternalDeviceLocked(Device* device) {
    if (device->configuration) {
        bool value;
...
bool EventHubZizhan::deviceHasMicLocked(Device* device) {
    if (device->configuration) {
        bool value;
...
int32_t EventHubZizhan::getNextControllerNumberLocked(Device* device) {
...
void EventHubZizhan::releaseControllerNumberLocked(Device* device) {
...
void EventHubZizhan::setLedForControllerLocked(Device* device) {
    for (int i = 0; i < MAX_CONTROLLER_LEDS; i++) {
        setLedStateLocked(device, ALED_CONTROLLER_1 + i, device->controllerNumber == i + 1);
    }
}

bool EventHubZizhan::hasKeycodeLocked(Device* device, int keycode) const {
...
status_t EventHubZizhan::mapLed(Device* device, int32_t led, int32_t* outScanCode) const {
    if (!device->keyMap.haveKeyLayout()) {
        return NAME_NOT_FOUND;
    }
...
void EventHubZizhan::closeDeviceByPathLocked(const char *devicePath) {
 ...
 */
void EventHubZizhan::closeVideoDeviceByPathLocked(const std::string& devicePath) {
...
void EventHubZizhan::closeAllDevicesLocked() {
...
void EventHubZizhan::closeDeviceLocked(Device* device) {
...
status_t EventHubZizhan::readNotifyLocked() {
    int res;
    char event_buf[512];
    int event_size;
    int event_pos = 0;
    ...
    ALOGV("EventHubZizhan::readNotify nfd: %d\n", mINotifyFd);
...
status_t EventHubZizhan::scanDirLocked(const char *dirname)
{
 ...
 */
status_t EventHubZizhan::scanVideoDirLocked(const std::string& dirname)
{
    DIR* dir;
    struct dirent* de;
...
void EventHubZizhan::requestReopenDevices() {
...
void EventHubZizhan::dump(std::string& dump) {
    ...
    { // acquire lock
        ...
        for (const std::unique_ptr<TouchVideoDeviceZizhan>& videoDevice : mUnattachedVideoDevices) {
