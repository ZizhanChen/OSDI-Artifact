...
//#include <hwcomposerzizhan/ComposerHalZizhan.h>
//#include <hwcomposerzizhan/HWC2Zizhan.h>
...
status_t BufferQueueProducer::queueBuffer(int slot,
    ...
    { // Autolock scope
        ...
        //zizhan
        //ALOGE("[zizhan] queueBuffer: slot=%d/%" PRIu64 " time=%" PRIu64 " dataSpace=%d"
        //        " validHdrMetadataTypes=0x%x crop=[%d,%d,%d,%d] transform=%#x scale=%s",
        //        slot, mCore->mFrameCounter + 1, requestedPresentTimestamp, dataSpace,
        //        hdrMetadata.validTypes, crop.left, crop.top, crop.right, crop.bottom,
        //        transform,
        ...
        /*
        //zizhan
        uint64_t idZizhan = mSlots[slot].mGraphicBuffer->getId();
        int32_t bufferIdZizhan = mSlots[slot].mGraphicBuffer->getBufferId();
        uint32_t widthZizhan = mSlots[slot].mGraphicBuffer->getWidth();
        uint32_t heightZizhan = mSlots[slot].mGraphicBuffer->getHeight();
        uint32_t strideZizhan = mSlots[slot].mGraphicBuffer->getStride();
        uint64_t usageZizhan = mSlots[slot].mGraphicBuffer->getUsage();
        uint32_t layerCountZizhan = mSlots[slot].mGraphicBuffer->getLayerCount();
        ALOGE("[zizhan] GraphicBuffer id=%" PRIu64 ", BufferId=%u, Width=%u, Height=%u,"
                "Stride=%u, Usage=%" PRIu64 ", LayerCount=%u", 
                idZizhan, bufferIdZizhan, widthZizhan, heightZizhan, strideZizhan, 
                usageZizhan, layerCountZizhan);
        
        ANativeWindowBuffer* nativeBufferZizhan = mSlots[slot].mGraphicBuffer->getNativeBuffer();
        int versionZizhan = nativeBufferZizhan->handle->version;
        int numFdsZizhan = nativeBufferZizhan->handle->numFds;
        int numIntsZizhan = nativeBufferZizhan->handle->numInts;
        ALOGE("[zizhan] NativeWindowBuffer version = %d, numFds=%d, numInts=%d", versionZizhan, numFdsZizhan, numIntsZizhan);
        
        mSlots[slot].mGraphicBuffer->dumpAllocationsToSystemLog();
        //getRenderEngine().drawLayers();
        */
